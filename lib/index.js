...
internals.redirectToSecureHttp = function (request, reply) {

  return reply.redirect('https://localhost:8001' + request.url.path).permanent();
};

internals.beforeApiResponse = function (request, reply) {
...
internals.beforeSecureHttpResponse = function (request, reply) {
...
exports.init = function (manifest, composeOptions, next) {
  ...
  Glue.compose(manifest, composeOptions, function (err, server) {
    ...
    server.select('web').ext('onRequest', internals.redirectToSecureHttp);
    server.select('web-tls').ext('onPreResponse', internals.beforeSecureHttpResponse);
    server.select('api').ext('onPreResponse', internals.beforeApiResponse);
    server.after(internals.after);
