...
internals.getValidateUser = function (username, password) {
  ...
  return Promise.resolve().then(function () {
    if (!username || !password) { throw {message: 'Did not submit password or username'}; }

    var account = Users[username] || null;
    if (!account || account.password !== password) { throw {message: 'Invalid password or username'}; }
    ...
    innerServer.route({
      ...
      config: {
        ...
        handler: function (request, reply) {
          ...
          return internals.getValidateUser(username, password).then(function (account) {
